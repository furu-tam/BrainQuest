...
import { getPathRecommendation } from "@/math/services/learningPath";
...
import { DAILY_QUESTION_COUNT, getModulesForGrade, type Grade } from "@/math/types/curriculum";

export default function MathHomePage() {
  const profile = useActiveMathProfile();
  const resetDailyIfNewDay = useMathStore((s) => s.resetDailyIfNewDay);
  const setActiveProfile = useMathStore((s) => s.setActiveProfile);
  const setStudentGrade = useMathStore((s) => s.setStudentGrade);
  const profiles = useMathStore((s) => s.profiles);
